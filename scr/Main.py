...
heuristic_solver = HeuristicBySolver(model, gap = 0.1, time_limit = 300)
...
lagrangian_relaxation = SurrogateLagrangianRelaxation(model, relaxed_constrs, mulpier, r = 0.8, big_m = 2)
lagrangian_relaxation.bulid_relaxed_duality(lagrangian_relaxation)
...
for k in range(max_iter_times):
    ...
    if lagrangian_relaxation.is_feasible():
        break
    lagrangian_relaxation.print_status()  
